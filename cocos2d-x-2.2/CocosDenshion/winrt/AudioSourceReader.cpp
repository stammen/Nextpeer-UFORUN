...
bool WAVReader::initialize(const std::string& filePath)
{
    ...
    do {
        auto fileSize = CCFileUtils::sharedFileUtils()->getFileSize(_filePath);
...
HRESULT MP3Reader::readAudioData(IMFSourceReader* pReader)
{
    ...
    do {
        ...
        if (createMappedCacheFile(_filePath, _mappedWavFile, ".dat")) {
            ...
            _audioSize = CCFileUtils::sharedFileUtils()->getFileSize(_mappedWavFile);
...
bool OGGReader::initialize(const std::string& filePath)
{
    ...
    do {
        ...
        if (ov_fopen(CCFileUtils::sharedFileUtils()->getSuitableFOpen(_filePath).c_str(), _vorbisFd.get())){
            break;
        }
